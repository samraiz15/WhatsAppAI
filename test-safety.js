...
async function testParkViewSafety() {
  ...
  const approvalText = String(approvalAnswer && approvalAnswer.answer || '').toLowerCase();
  ...
  const unknownBlockText = String(unknownBlockAnswer && unknownBlockAnswer.answer || '').toLowerCase();
  assert.ok(
  ...
  const staleSourceText = String(staleSourceAnswer && staleSourceAnswer.answer || '').toLowerCase();
  assert.ok(
  ...
  const conflictingSourceText = String(conflictingSourceAnswer && conflictingSourceAnswer.answer || '').toLowerCase();
  assert.ok(
  ...
  const priceText = String(priceAnswer && priceAnswer.answer || '').toLowerCase();
